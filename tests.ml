...
let test_valid_residence () =
  let my_address = {mailbox=1201; street=MassAve; zip_code="02138"} in
  let my_house = House my_address in
  unit_test((valid_residence my_house) = true) "valid_residence  true";

  let bad_address = {mailbox= ~-12; street=MassAve; zip_code="02138"} in
...
let test_neighbors () =
  ...
  let far_away_address = {mailbox=1201; street=MainStreet; zip_code="02138"} in
  let my_house = House my_address in
  ...
  let diff_zip_address = {mailbox=1201; street=MassAve; zip_code="45140"} in
  let my_house = House my_address in
...
let  test_close_to_seven () =
  let close_address = {mailbox = 8; street = MassAve ; zip_code = "02138"} in
  let close_res = House close_address in
  let far_address  = {mailbox = 3; street = MassAve; zip_code = "02138"} in
...
let test_choose_residence () =
  let my_address = {mailbox=9; street=MassAve; zip_code="02138"} in
  let my_house = House my_address in
  let neighbor_house = House my_address in

  let far_away_address = {mailbox=12; street=MainStreet; zip_code="02138"} in
  let not_neighbor_house = Apartment (1, far_away_address) in

  let diff_zip_address = {mailbox=6; street=MassAve; zip_code="45140"} in
...
let test_named_neighbors () =
  let my_address = {mailbox=1; street=MassAve; zip_code="02138"} in
  let my_house = House my_address in
  let neighbor_house = House my_address in
  ...
  let neighbor_address = {mailbox=1; street=MassAve; zip_code="02138"} in
